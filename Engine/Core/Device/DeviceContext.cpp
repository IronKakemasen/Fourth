...
void DeviceContext::SetDebugLayerFilter()
{
#if defined(_DEBUG)
...
void DeviceContext::ShaderModelChack(D3D_SHADER_MODEL shaderModel_)
{
	...
	ErrorMessageOutput::Abort::DetectError((SUCCEEDED(hr)) || shaderModel.HighestShaderModel >= D3D_SHADER_MODEL_6_5,
		"ShaderModelが6_5以上を対応していません。", "DeveiceContext.cpp");	
}

void DeviceContext::IsMeshShaderSupported()
{
	...
	ErrorMessageOutput::Abort::DetectError((SUCCEEDED(hr)) || features.MeshShaderTier == D3D12_MESH_SHADER_TIER_NOT_SUPPORTED,
		"ShaderModelがサポートされていません。", "DeveiceContext.cpp");
}

void DeviceContext::FetchAdapter()
{	
	//一番いいアダプタを頼む
		...
		DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE, IID_PPV_ARGS(&useAdapter)) != DXGI_ERROR_NOT_FOUND; ++i)
	{
		...
		ErrorMessageOutput::Abort::DetectError((SUCCEEDED(hr)), "アダプタの取得に失敗しました", "DeviceContext.cpp");
	...
	ErrorMessageOutput::Abort::DetectError((useAdapter != nullptr), "適切なアダプタが見つからなかった", "DeviceContext.cpp");
}

void DeviceContext::CreateDXGI_Factory()
{
	HRESULT hr = CreateDXGIFactory(IID_PPV_ARGS(&dxgiFactory));
	//初期化の根本的な部分でエラーが出た場合はプログラムが間違っているか、
	//どうにもできない場合が多いのでassertにしておく
	ErrorMessageOutput::Abort::DetectError(SUCCEEDED(hr), "DXGIFactoryの生成に失敗しました", "DeviceContext.cpp");
...
void DeviceContext::CreateDevice()
{
	...
	ErrorMessageOutput::Abort::DetectError(device != nullptr, "でバイスの生成が上手くいかなかったので起動できない", "DeviceContext.cpp");
